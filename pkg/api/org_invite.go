...
func fileLoad(c *models.ReqContext) response.Response {
	...
	for i := 0; i < uid.Len(); i++ {
		...
		eqType, _ := dash.Data.Get("eqtype").String()
		...
		eqType_arr = append(eqType_arr, eqType)
	}
...
func pythonCmd(c *models.ReqContext) response.Response {
	body, err := c.Req.Body().Bytes()
	if err != nil {
		return response.JSON(400, "err1")
	}
	...
	table := fmt.Sprintf("%v", reflect.ValueOf(param["Table"]))
	fab := fmt.Sprintf("%v", reflect.ValueOf(param["Fab"]))
	eqtype := fmt.Sprintf("%v", reflect.ValueOf(param["EqType"]))
	eqname := fmt.Sprintf("%v", reflect.ValueOf(param["EqName"]))
	inout := fmt.Sprintf("%v", reflect.ValueOf(param["IO"]))
	stage := fmt.Sprintf("%v", reflect.ValueOf(param["Stage"]))
	ip := fmt.Sprintf("%v", reflect.ValueOf(param["IP"]))
	id := fmt.Sprintf("%v", reflect.ValueOf(param["ID"]))

	//cmd := exec.Command("python", "../data/dashboard_generator_API.py", table, fab, eqtype, eqname, inout, stage, ip, id)
	cmd := exec.Command("python3.7", "/root/grafana/data/dashboard_generator_API.py", table, fab, eqtype, eqname, inout, stage, ip, id)

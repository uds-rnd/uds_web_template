...
func apiMiddleware(c *models.ReqContext) response.Response {
	...
	_url := fmt.Sprintf("%v", reflect.ValueOf(param["url"]))
	_param := param["param"]

	pbytes, _ := json.Marshal(_param)
	buff := bytes.NewBuffer(pbytes)
	resp, err := http.Post(_url, "application/json", buff)
	...
	if err != nil {
		return response.JSON(400, "err4")
	}

	return response.JSON(resp.StatusCode, respBody)
}

...
// @ts-ignore
import { convertToRankingData } from './rankingchart';
...
class GraphElement {
  ...
  onPlotSelected(event: JQueryEventObject, ranges: any) {
    ...
    if ((ranges.ctrlKey || ranges.metaKey) && this.dashboard.canAddAnnotations()) {
      ...
      this.scope.$apply(() => {
        if (ranges.yaxis) {
          this.panel.yaxes[0].min = ranges.yaxis.from;
          this.panel.yaxes[0].max = ranges.yaxis.to;
        }
        if (ranges.y2axis) {
          this.panel.yaxes[1].min = ranges.y2axis.from;
          this.panel.yaxes[1].max = ranges.y2axis.to;
        }
        this.zoom = true;
  ...
  prepareXAxis(options: any, panel: any) {
    switch (panel.xaxis.mode) {
      ...
      case 'rankingchart': {
        ...
        if (this.data.length) {
          this.data = convertToRankingData(this.data, this.ctrl.hiddenSeries);

          let histMin = _.min(_.map(this.data, (s) => s.stats.min));
          let histMax = _.max(_.map(this.data, (s) => s.stats.max)) + 1;
          const ticks = panel.xaxis.buckets || 4;
          bucketSize = tickStep(histMin, histMax, ticks);
          options.series.bars.barWidth = bucketSize * 0.4;
        } else {
          bucketSize = 0;
        }

        this.addXRankingChartAxis(options, bucketSize);
        break;
      }
      case 'table': {
        options.series.bars.barWidth = 0.7;
  ...
  addTimeAxis(options: any) {
    ...
    _.map(this.data, (series, index) => {
      if (min === this.ctrl.range.from.valueOf()) {
        min = series.stats.minTime;
      } else {
        if (min! > series.stats.minTime && series.stats.minTime > this.ctrl.range.from.valueOf()) {
          min = series.stats.minTime;
        }
      }
      if (max === this.ctrl.range.to.valueOf()) {
        max = series.stats.maxTime;
      } else {
        if (max! < series.stats.maxTime && series.stats.maxTime < this.ctrl.range.to.valueOf()) {
          max = series.stats.maxTime;
        }
      }
    });
  ...
  addXRankingChartAxis(options: any, bucketSize: number) {
    let ticks, min, max, ticks_label;
    let defaultTicks = this.panelWidth / 50;
    if (this.data.length && bucketSize) {
      let tick_values = [];
      let tick_label_values = [];
      for (let d of this.data) {
        let i = 0;
        for (let point of d.data) {
          tick_values[point[0]] = true;
          tick_label_values[i++] = point[3];
        }
      }
      ticks = Object.keys(tick_values).map((v) => Number(v));
      ticks_label = tick_label_values;
      min = _.min(ticks)!;
      max = _.max(ticks)!;
      // Adjust tick step
      let tickStep = bucketSize;

...
class DashNav extends PureComponent<Props> {
  ...
  renderDashboardTitleSearchButton() {
    ...
    if (dashboard.meta.folderUrl && dashboard.meta.folderUrl.length > 0) {
      const _strArr = dashboard.meta.folderUrl.split('/');
      if (_strArr.length >= 2) {
        let uid = [_strArr[_strArr.length - 2]];
        let ii = getBackendSrv()
          .post('/fileload', { uid })
          .then((res: any) => {
            if (res.filename.length > 0 && res.filename[0] !== '') {
              this.setState({
                ...this.state,
                fileName: res.filename[0] + ' / ',
              });
            }
          });
      }
            ...
            {haveFolder && (
              <>
                <a className="navbar-page-btn__folder" onClick={this.onFolderNameClick}>
                  {this.state.fileName} {folderTitle} <span className={folderSymbol}>/</span>
                </a>
              </>
            )}

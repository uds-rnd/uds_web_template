...
import { getValueFormat, ValueFormatter, DecimalCount, formattedValueToString } from '@grafana/data';
...
export default class TimeSeries {
  ...
  applySeriesOverrides(overrides: any[]) {
    ...
    for (let i = 0; i < overrides.length; i++) {
      ...
      //   continue;
      // }
      if (override.alias !== this.metricName) {
        continue;
      }
  ...
  getFlotPairs(fillStyle: string) {
    ...
    this.allIsNull = true;
    this.allIsZero = true;
    this.stats.minTime = Number.MAX_VALUE;
    this.stats.maxTime = -Number.MAX_VALUE;
    ...
    for (let i = 0; i < this.datapoints.length; i++) {
      ...
      if (currentValue !== null) {
        ...
        if (currentTime > this.stats.maxTime) {
          this.stats.maxTime = currentTime;
        }

        if (currentTime < this.stats.minTime) {
          this.stats.minTime = currentTime;
        }

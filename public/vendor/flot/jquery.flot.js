  ...
  function Plot(placeholder, data_, options_, plugins) {
    ...
    function processData() {
      ...
      for (i = 0; i < series.length; ++i) {
        ...
        tooltip = s.datapoints.tooltip || null;
              ...
              if (f) {
                ...
                if (val == null) {
                  ...
                  if (p[m] && tooltip) {
                    for( var key in p[m] ) {  
                      if (tooltip[k]) {
                        tooltip[k].push([key, (p[m])[key]]);  
                      } else {
                        tooltip[k] = [[key, (p[m])[key]]];
                      }
                    } 
                  }
                }
    ...
    function setTicks(axis) {
      ...
      if (oticks == null || (typeof oticks == 'number' && oticks > 0)) ticks = axis.tickGenerator(axis);
        ...
        if ($.isFunction(oticks))
          ...
          ticks = oticks;
          ticks_label = axis.options.ticks_label;
        }
      }
      ...
      for (i = 0; i < ticks.length; ++i) {
        ...
        if (label == null) {
          if (ticks_label && ticks_label.length > 0) {
            label = ticks_label[i];
          } else {
    ...
    function drawTop_Bar(x, y, b, barLeft, barRight, fillStyleCallback, axisx, axisy, c, horizontal, lineWidth) {
      var left, right, bottom, top, drawLeft, drawRight, drawTop, drawBottom, tmp;
      // in horizontal mode, we start the bar from the left
      // instead of from the bottom so it appears to be
      // horizontal rather than vertical
      ...
      if (fillStyleCallback) {
        // c.fillStyle = fillStyleCallback(bottom, top);
        c.fillStyle = custom_bar_color[x];
        c.fillRect(left, top, right - left, bottom - top);
        if (y !== 0) {
          c.fillStyle = 'white';
          c.fillText(y, (left + right) / 2 - 5, top - 5, right - left);
        }
      }
    ...
    function drawSeriesBars(series) {
      function plotBars(datapoints, barLeft, barRight, fillStyleCallback, axisx, axisy) {
        ...
        for (var i = 0; i < points.length; i += ps) {
          ...
          if (axisx.options.label === 'RangkingChart') {
            drawTop_Bar(
              points[i],
              points[i + 1],
              points[i + 2],
    ...
    function insertLegend() {
      ...
      var table =
        '<table style="font-size:smaller;color:' + options.grid.color + '">' + fragments.join('') + '</table>';
    ...
    function findNearbyItem(mouseX, mouseY, seriesFilter) {
      ...
      if (item) {
        ...
        if (series[i].datapoints.tooltip) {
          return {
            datapoint: series[i].datapoints.points.slice(j * ps, (j + 1) * ps),
            dataIndex: j,
            series: series[i],
            seriesIndex: i,
            tooltip: series[i].datapoints.tooltip[j * ps],
          };
        } else {
          return {
            datapoint: series[i].datapoints.points.slice(j * ps, (j + 1) * ps),
